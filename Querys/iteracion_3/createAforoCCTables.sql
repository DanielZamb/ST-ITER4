...
( 
     ...
     color              VARCHAR (50 byte) NOT NULL,
     tipo_lugar         NUMBER, 
     CONSTRAINT a_espacio_pk PRIMARY KEY (id) 
  ); 
ALTER TABLE A_ESPACIO
ADD CONSTRAINT CK_COLOR_ESPACIO
CHECK (color IN('Verde','Naranja','Rojo'))
ENABLE;
...
CREATE TABLE a_establecimiento 
  ( 
     ...
     tipo_establecimiento VARCHAR(255 byte) NOT NULL,
     id_cc   NUMBER, 
...
CREATE TABLE a_visitante 
  ( 
     ...
     positivo            VARCHAR(1) NOT NULL,
     color               VARCHAR(50) NOT NULL,
     tipo                NUMBER, 
     CONSTRAINT a_visitante_pk PRIMARY KEY (id) 
  ); 
ALTER TABLE A_VISITANTE
ADD CONSTRAINT CK_POSITIVO
CHECK (positivo IN ('1','0'))
ENABLE;
ALTER TABLE A_VISITANTE
ADD CONSTRAINT CK_COLOR_VISITANTE
CHECK (color IN('Verde','Naranja','Rojo'))
ENABLE;
...
CREATE TABLE a_visitas 
  ( 
     ...
     CONSTRAINT a_visitas_pk PRIMARY KEY (id_visitante, hora_ingreso) 
  );
